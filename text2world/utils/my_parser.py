...
class PDDLDomainParser(PDDLParser, PDDLDomain):
    ...
    """
    def __init__(self, domain=None, operators_as_actions=False, expect_action_preds=True):
        # Parsing sets all domain fields
        PDDLDomain.__init__(self, operators_as_actions=operators_as_actions)
        self.operators_as_actions = operators_as_actions
        self.expect_action_preds = expect_action_preds
        self.domain = domain
        if domain is not None:
            self.domain = domain.lower()

    def test_exec(self, domain=None):
        if domain is not None:
            self.domain = domain.lower()
        ...
        if self.operators_as_actions:
            assert not self.expect_action_preds
            self.actions = self._create_actions_from_operators()
        elif not self.expect_action_preds:
            self.actions = set()
